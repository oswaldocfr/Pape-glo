<div class="w-full">
    ...
    <div class="pt-4 border-t justify-items-end w-full">
        ...
        @endphp
        <table class=" w-full table text-right">
            @foreach ($feeNames as $key => $feeName)
                <tr class="border-b text-right w-full">
                    <td class="p-2 text-right">
                        {{ $feeName }}
                    </td>
                    <td class="p-2 text-right w-36">
                        <x-details.p text="{{ $fees[$key] }}" />
                    </td>
                </tr>
            @endforeach
        ...
        <table class=" w-full table text-right">
            <tr class="border-b text-right w-full">
                <td class="p-2 text-right flex justify-end items-end">
                    <x-label title="{{ __('Total') }}" />
                </td>
                <td class="p-2 text-right w-36">
                    <x-de<x-details.p text="{{ currencyFormat($selectedModel->total ?? '') }}" />
                </td>
            </tr>
        </table>
    </div>
