<x-form noClass="true" action="saveDriverSettings">
    <div class='grid grid-cols-1 gap-4 mb-10 md:grid-cols-2 lg:grid-cols-3'>
        ...
        <div class="block mt-4 text-sm {{ $autoassignmentsystem == 0 ? '' : 'hidden' }}">
            ...
            <div class="mt-1 text-xs {{ $fetchNearbyDriverSystem == 1 ? '' : 'hidden' }}">
                <p class="mb-1">
            ...
            {{-- websocket --}}
            <div class="mt-1 text-xs {{ $fetchNearbyDriverSystem == 2 ? '' : 'hidden' }}">
                <p class="mb-1">
                    <span class="font-bold">{{ __('Websoket') }}:</span>
                    {{ __('On Server Websocket') }}
                </p>
                <div class="space-y-2">
                    <div>
                        <p class="font-bold underline">{{ __('Pros') }}</p>
                        <ul>
                            <li>
                                <strong>{{ __('Self-hosted & Scalable:') }}</strong>
                                {{ __('No external vendor lock-in; you control scaling and infrastructure.') }}
                            </li>
                            <li>
                                <strong>{{ __('Low Latency:') }}</strong>
                                {{ __('Real-time messaging with minimal delay') }}
                            </li>
                            <li>
                                <strong>{{ __('Cheaper:') }}</strong>
                                {{ __('There is no price on number of read and writes') }}
                            </li>
                        </ul>
                    </div>
                    <div>

                        <p class="font-bold underline">{{ __('Cons') }}</p>
                        <ul>
                            <li>
                                <strong>{{ __('More DevOps Overhead:') }}</strong>
                                {{ __('Requires managing your own WebSocket server (Reverb,SSL, scaling).') }}
                            </li>
                            <li>
                                <strong>{{ __('No Offline Sync:') }}</strong>
                                {{ __("Missed events aren't retained") }}
                            </li>
                        </ul>
                    </div>
