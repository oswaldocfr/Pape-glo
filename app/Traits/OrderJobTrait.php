...
use App\Jobs\WebsocketRegularOrderMatchingJob;
...
trait OrderJobTrait
{
    ...
    public function handleRegularAssignmentJob(Order $order)
    {
        ...
        if ($useFCMJob && $order->vendor != null) {
            ...
            //if its websocket
            if ($fetchNearbyDriverSystem == 2) {
                if (delayFCMJob()) {
                    WebsocketRegularOrderMatchingJob::dispatch($order)->delay($delayFor);
                } else {
                    WebsocketRegularOrderMatchingJob::dispatch($order);
                }
            } else {
                if (delayFCMJob()) {
                    RegularOrderMatchingJob::dispatch($order)->delay($delayFor);
                } else {
